    ...
    */
    var drawWords = function (el, words) {
      ...
      for (var i = 0, len = words.length; i < len; i++) {
        var word = words[i];
        output += '<li style="font-family: Century Gothic;" class="word ' + word + '">' + word;
      }
      output += '</ul>';
      ...
      for (var i = 0, len = wordList.length; i < len; i++) {
        ...
        if (wordList[i] === curWord) {
          ...
          $('.' + curWord).addClass('wordFound');
            ...
            var hh = words_def[curWord];
            
            var hhh = words_photo[curWord];
            if(hhh!=""){}else{ hhh="no_photo.png"; }
            //swal({ html:true, title:'<i>Facts</i>', text:hh});
            document.getElementById("def").innerHTML = "<div style='text-align: justify; text-justify: inter-word;'><p style='font-size: 140%;'><img  src='http://202.90.141.83/helponline/assets/images/puzzle_photo/"+hhh+"'/> "+curWord+":"+hh+"</p></div>";
        }
