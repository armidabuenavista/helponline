...
body {
    background: linear-gradient(-70deg, #fa7c30 30%, rgba(0, 0, 0, 0) 30%), url('<?php echo base_url(); ?>assets/images/puzzle/image1.png');
...
<div class="sample container">
    ...
    <h1 class="header_sub">GO FOODS</h1>
    <div id='puzzle'></div>
    <div id='words'></div>  
    <div id="def"></div>
</div>
...
<?php $this->load->model('mdl_help', '', TRUE); foreach ($this->load->mdl_help->get_record("1") as $value) { echo 'words_def["'.str_replace(" ","",$value->name).'"]="'.$value->facts.'";'; }?>
  ...
  var words_photo = [""];
<?php $this->load->model('mdl_help', '', TRUE); foreach ($this->load->mdl_help->get_record("1") as $value) { echo 'words_photo["'.str_replace(" ","",$value->name).'"]="'.$value->photo.'";'; }?>
